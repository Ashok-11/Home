"""Gemini-powered household intelligence, via the Emergent universal LLM key.
...
def _chat(session_id: str, system_message: str) -> LlmChat:
    return (
        LlmChat(api_key=_api_key(), session_id=session_id, system_message=system_message)
...
async def parse_expenses(body: ParseExpensesRequest, _: dict = Depends(require_user)):
    ...
    chat = _chat(f"parse-expenses-{uuid.uuid4()}", system)
    raw = _text(await chat.send_message(UserMessage(text=body.text)))
...
async def recipe_ideas(body: RecipeIdeaRequest, _: dict = Depends(require_user)):
    ...
    chat = _chat(f"recipe-ideas-{uuid.uuid4()}", system)
    raw = _text(await chat.send_message(UserMessage(text=user_text)))
...
async def plan_menu(body: PlanMenuRequest, _: dict = Depends(require_user)):
    ...
    chat = _chat(f"plan-menu-{uuid.uuid4()}", system)
    raw = _text(await chat.send_message(UserMessage(text=f"Plan the week starting {body.start_date}.")))
...
async def copilot(body: CopilotRequest, user: dict = Depends(require_user)):
    ...
    async def event_stream():
        ...
        full: list[str] = []
        chat = _chat(f"copilot-{uuid.uuid4()}", system)
        try:
            async for ev in chat.stream_message(UserMessage(text=body.message)):
                if isinstance(ev, TextDelta):
                    ...
                    break
        except Exception as exc:  # surface provider errors as a readable SSE event
